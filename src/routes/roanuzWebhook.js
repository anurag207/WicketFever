...
// ⛔️ Do NOT use bodyParser.json() here; we need raw bytes for gzip
// router.use(bodyParser.json());

router.post(
  '/match/feed/v1',
  // ✅ Route-scoped raw parser so we can unzip gzipped payloads
  express.raw({ type: '*/*', limit: '2mb' }),
  async (req, res) => {
    const startedAt = Date.now();
    ...
    // Debug: confirm route hit and raw body presence
    console.log('--- Roanuz Webhook START -----------------------------');
    console.log('[Roanuz WH] Path:', req.originalUrl);
    console.log('[Roanuz WH] Content-Encoding:', req.headers['content-encoding']);
    console.log('[Roanuz WH] Raw bytes length:', req.body?.length ?? 0);

    // 2) Try to unzip; fallback to plain JSON if not compressed
    const zipped = Buffer.isBuffer(req.body) ? req.body : Buffer.from(req.body || []);
    zlib.unzip(zipped, async (unzipErr, buffer) => {
      let payload;
      if (unzipErr) {
        console.warn('[Roanuz WH] unzip failed, trying plain JSON:', unzipErr.message);
        try {
          payload = JSON.parse(zipped.toString('utf8'));
          console.warn('[Roanuz WH] Parsed as plain JSON (no gzip)');
        } catch (e) {
          console.error('[Roanuz WH] ❌ Invalid payload (not gzip nor JSON):', e.message);
        ...
        try {
          ...
          console.error('[Roanuz WH] ❌ JSON parse error:', e.message);
          return res.status(400).json({ status: false, error: 'Invalid JSON' });
        }
      }
      ...
      const matchData = payload?.data?.key ? payload.data : payload;
      const matchKey = matchData?.key;
      console.log('[Roanuz WH] Match key:', matchKey, 'Status:', matchData?.status);

      if (!matchKey) {
        console.warn('[Roanuz WH] ⚠️ Missing match key; acking to avoid retries');
        console.log('--- Roanuz Webhook END (missing key) ---------------');
        return res.status(200).json({ status: true });
      }

      // 4) Build detailed scorecard
      let detailedScorecard = null;
      try {
        detailedScorecard = buildDetailedScorecard(matchData);
        console.log('[Roanuz WH] ✅ built detailed scorecard');
      } catch (e) {
        console.error('[Roanuz WH] ❌ buildDetailedScorecard failed:', e.message);
      }
      ...
      try {
        ...
        console.log('[Roanuz WH] ✅ Redis set for', matchKey);
      } catch (e) {
        console.error('[Roanuz WH] ❌ Redis error:', e.message);
      }
      ...
      try {
        ...
        console.log('[Roanuz WH] ✅ Socket emit for', matchKey);
      } catch (e) {
        console.error('[Roanuz WH] ❌ Socket emit error:', e.message);
      }
      ...
      try {
        ...
        console.log('[Roanuz WH] ✅ Mongo upsert for', matchKey);
      } catch (e) {
        console.error('[Roanuz WH] ❌ Mongo upsert error:', e.message);
      }

      // 8) ACK
      const tookMs = Date.now() - startedAt;
      console.log('[Roanuz WH] ✅ ACK 200 (ms):', tookMs);
      console.log('--- Roanuz Webhook END -----------------------------');
      return res.status(200).json({ status: true });
    });
  }
